...
namespace Filter
{
    ...
    inline double MitchellNetravali_custom(double x, const double B, const double C)
    {
    ...
    inline double MitchellNetravali_default(double x)
    {
        return MitchellNetravali_custom(x, 1.0/3.0, 1.0/3.0);
    }

    inline double CatmullRom(double x)
    {
        return MitchellNetravali_custom(x, 0.0, 0.5);
    }

    inline double BSpline(double x)
    {
        return MitchellNetravali_custom(x, 1.0, 0.0);
    }

    inline double Hermite(double x)
    {
        // Stretch by 2 since Hermite filter loses support for x > 1
        return MitchellNetravali_custom(x * 0.5, 0.0, 0.0);
    }
